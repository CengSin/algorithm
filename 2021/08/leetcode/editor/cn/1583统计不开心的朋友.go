...
func unhappyFriends(n int, preferences [][]int, pairs [][]int) (ans int) {
	preferencesMap := make(map[int]map[int]int)
	pairsMap := make(map[int]int)

	for x, friends := range preferences {
		if _, ok := preferencesMap[x]; !ok {
			preferencesMap[x] = make(map[int]int)
		}
		for index, l := range friends {
			preferencesMap[x][l] = index
		}
	}
	...
	for x, y := range pairsMap {
		yorder := preferencesMap[x][y]
		for _, index := range preferences[x][:yorder] {
			if preferencesMap[index][pairsMap[index]] > preferencesMap[index][x] {
				ans++
				break
			}
		}
